...
사용자는 그동안 빈 화면을 보며 기다려야 한다. 로딩 스피너가 돌고 있겠지만... 결과가 나오기까지 사용자가 기다려야 하기 때문에 사용 경험도 좋지 않고 이탈의 가능성도 높아진다.
...
이제 스트림을 요청하면 된다. 여기서 핵심은 `stream: true` 옵션이다.
...
\* 참고:  Delta는 **"변화량"**을 의미하는 단어인데, 스트리밍에서는 이전과 비교해서 추가되는 데이터를 이야기한다.
